### Purpose
The code contained here is an implementation of a label propagation algorithm that calculates the number of strongly connected components in a graph, represented by a matrix, which is in the compressed sparse row (CSR) format. In this branch, the basic algorithm is parallelized using Pthreads/POSIX Threads. Place a `.mat` file in the same directory as the source code and follow the steps below to run the program and calculate the number of strongly connected components of the graph this specific matrix represents.
### Dependencies 
